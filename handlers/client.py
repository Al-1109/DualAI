...
async def send_welcome_to_channel(context):
    ...
    Упрощенная версия, которая просто отправляет сообщение без закрепления.
    """
    ...
    try:
        ...
        logger.info(f"Отправлено текстовое приветственное сообщение (ID: {message.message_id})")
        return message
