...
- Доступны два режима работы бота:
  1. Long Polling (локально, через `test_bot.py`) - полностью рабочий режим, но только для локальной разработки
  2. Webhook (на Vercel, через `api/webhook.js`) - полностью рабочий режим для промышленной эксплуатации
...
- **Python** - используется для локальной разработки и тестирования
...
- Продакшен: webhook на Vercel через `api/webhook.js`

## Взаимодействие компонентов
1. Пользователь отправляет сообщение боту в Telegram
2. В режиме long polling (локально): Python-скрипт получает обновление через Telegram API
...
- Long polling более надежен для разработки и тестирования, но не подходит для Vercel
...
5. Webhook предоставляет большую масштабируемость, но требует публично доступного HTTPS-сервера
